-- Complete role-based access system setup
...
-- Feature flags policies - admin/owner can manage, everyone can read
CREATE POLICY "feature_flags_read_all" ON public.feature_flags
  FOR SELECT USING (true);

CREATE POLICY "feature_flags_admin_owner_write" ON public.feature_flags
  FOR INSERT WITH CHECK (is_admin_or_owner());

CREATE POLICY "feature_flags_admin_owner_update" ON public.feature_flags
...
INSERT INTO public.feature_flags (key, enabled, description) VALUES
  ('user_dashboard', true, 'Användar-dashboard'),
  ('admin_dashboard', true, 'Admin-dashboard'),
  ('staff_management', true, 'Personalhantering'),
  ('advanced_reporting', true, 'Avancerad rapportering')
ON CONFLICT (key) DO NOTHING;