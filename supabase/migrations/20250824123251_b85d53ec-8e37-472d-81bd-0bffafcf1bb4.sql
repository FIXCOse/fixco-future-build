-- Admin system tables and views for OWNER/ADMIN functionality

-- Add role column to profiles if not exists (already exists from previous migration)
-- ALTER TABLE profiles ADD COLUMN IF NOT EXISTS role text CHECK (role IN ('customer','staff','admin','owner')) DEFAULT 'customer';
...
SELECT 
  date_trunc('month', created_at) AS month,
  sum(rot_amount + rut_amount) AS savings
FROM public.bookings
WHERE rot_amount > 0 OR rut_amount > 0
GROUP BY 1
ORDER BY 1 DESC
LIMIT 12;
...
-- Enable RLS on views for OWNER/ADMIN
ALTER VIEW public.vw_bookings_daily SET (security_barrier = true);
ALTER VIEW public.vw_revenue_monthly SET (security_barrier = true);  
ALTER VIEW public.vw_rot_rut_savings_monthly SET (security_barrier = true);
ALTER VIEW public.vw_top_services SET (security_barrier = true);
