...
const JobPool = () => {
  ...
  const loadJobs = async () => {
    try {
      ...
      console.log('JobPool - Loaded jobs:', data?.length, 'jobs');
      console.log('🔍 Bonus data:', data.map(j => ({ 
        id: j.id, 
        title: j.title, 
        bonus_amount: j.bonus_amount,
        admin_set_price: j.admin_set_price 
      })));
      setJobs(data);
    } catch (error) {
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                    ...
                    {job.admin_set_price && job.admin_set_price > 0 && (
                      <div className="bg-gradient-to-r from-yellow-100 to-orange-100 border-2 border-yellow-500 rounded-lg px-3 py-2">
                        <div className="flex items-center gap-2">
                          <Gift className="w-5 h-5 text-yellow-600 animate-pulse flex-shrink-0" />
                          <div>
                            <p className="text-xs font-semibold text-yellow-800 uppercase">EXTRA BONUS</p>
                            <p className="text-lg font-bold text-yellow-900">+{job.admin_set_price} kr</p>
                          </div>
                        </div>
                      </div>
                    )}
