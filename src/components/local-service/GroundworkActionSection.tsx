...
import { Shield, Percent, Mountain } from "lucide-react";
import groundworkImage from "@/assets/markarbeten-gravmaskiner.webp";

interface GroundworkActionSectionProps {
  area: string;
}

export const GroundworkActionSection = ({ area }: GroundworkActionSectionProps) => {
  const usps = [
    { icon: Shield, text: "Erfarna entreprenörer" },
    { icon: Percent, text: "ROT-avdrag 30%" },
    { icon: Mountain, text: "Grävmaskiner" },
  ];
  ...
  return (
    ...
    >
      ...
      <img
        src={groundworkImage}
        alt={`Fixco markarbeten med grävmaskiner i ${area}`}
