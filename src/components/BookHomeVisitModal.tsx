...
import { X, ArrowRight, ArrowLeft, CheckCircle, Home, Clock, MapPin, User } from 'lucide-react';
...
const BookHomeVisitModal = () => {
  ...
  const projectTypes = [
    { id: 'el', name: 'El', icon: '⚡', desc: 'Elmontör' },
    { id: 'vvs', name: 'VVS', icon: '🚿', desc: 'VVS-installation' },
    { id: 'snickeri', name: 'Snickeri', icon: '🔨', desc: 'Snickare' },
    { id: 'malning', name: 'Målning', icon: '🎨', desc: 'Målare' },
    { id: 'montering', name: 'Montering', icon: '🔧', desc: 'Montör' },
    { id: 'stadning', name: 'Städning', icon: '✨', desc: 'Städtjänster' },
    { id: 'tradgard', name: 'Trädgård', icon: '🌿', desc: 'Trädgårdsskötsel' },
    { id: 'mark', name: 'Mark', icon: '⛏️', desc: 'Markarbeten' }
  ];

  const desiredTimes = [
    { id: 'asap', name: 'Så snart som möjligt', icon: '🚨', desc: 'Akut behov' },
    { id: '1-2days', name: 'Inom 1-2 dagar', icon: '⚡', desc: 'Snabbt besök' },
    { id: 'week', name: 'Inom en vecka', icon: '📅', desc: 'Ingen brådska' },
    { id: 'month', name: 'Nästa månad', icon: '🗓️', desc: 'Planerat projekt' }
  ];
  ...
  const handleSubmit = async () => {
    ...
    try {
      const selectedProject = projectTypes.find(p => p.id === wizardData.projectType);
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {projectTypes.map(type => (
                    <button
                      key={type.id}
                      onClick={() => setWizardData({...wizardData, projectType: type.id})}
                      className={cn(
                  ...
                  <div className="space-y-1 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {projectTypes.find(p => p.id === wizardData.projectType)?.name}
                      </span>
                    </div>
                    <div className="flex justify-between">
