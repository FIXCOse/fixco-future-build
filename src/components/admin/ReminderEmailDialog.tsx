...
import { Sparkles, Send, Loader2, Bell } from "lucide-react";
...
export function ReminderEmailDialog({ open, onOpenChange, quoteId, customerName, locale, onSuccess }: ReminderEmailDialogProps) {
  ...
  const handleGenerateText = async (type: 'subject' | 'body') => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('generate-reminder-text', {
      ...
      toast.success(type === 'subject' ? 'Ämnesrad genererad!' : 'Påminnelsetext genererad!');
    } catch (error: any) {
  ...
  const handleSend = async (testEmail?: string) => {
    ...
    try {
      ...
      toast.success(testEmail ? `Testmail skickat till ${testEmail}! 🧪` : 'Påminnelsemail skickat! 🔔');
      if (!testEmail) {
        onOpenChange(false);
        setSubject("");
        setBody("");
      ...
      toast.error(error.message || 'Kunde inte skicka påminnelsemail');
    } finally {
      setSending(false);
    }
  };
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Påminnelsemail till {customerName}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800">
            <p className="font-medium">🔔 Detta är en påminnelse — inte ett uppföljningsmail</p>
            <p className="mt-1 text-amber-700">Mailet har en tydlig påminnelse-design (orange/gul) och nämner offertens giltighetstid om den finns.</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="T.ex. 'Betona att offerten går ut snart' eller 'Kort och rakt på sak'"
              className="text-sm"
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Input
                ...
                placeholder="T.ex. 'Påminnelse: Din offert från Fixco'"
                className="flex-1"
              />
              <Button
                variant="outline"
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              >
                ...
                AI-generera påminnelse
              </Button>
            </div>
            <Textarea
              value={body}
              onChange={(e) => setBody(e.target.value)}
              placeholder="Skriv din påminnelsetext här eller generera med AI..."
          ...
          {body && (
            <div className="p-3 bg-muted rounded-lg text-sm text-muted-foreground">
              ...
              <p>Kunden får en orange/gul påminnelse-mall med din text, giltighetstid och en knapp till offertsidan.</p>
            </div>
          )}
        </div>
